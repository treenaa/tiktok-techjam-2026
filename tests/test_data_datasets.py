...
def test_sample_carries_optional_metadata(records):
    ...
    assert "generator" in sample and "index" in sample and "transform_name" in sample
...
def test_subset_preserves_settings(records):
    dataset = ManifestDataset(records, transform="jpeg_30")
    subset = dataset.subset([0, 1, 2])
    assert len(subset) == 3 and subset.transform.name == "jpeg_30"


# -- transforms inside the dataset ----------------------------------------
def test_transform_can_be_a_name_or_a_callable(records):
    by_name = ManifestDataset(records, transform="blur_2.0")[0]["image"]
    ...
    assert ManifestDataset(records, transform="blur_2.0")[0]["transform_name"] == "blur_2.0"


def test_transform_changes_pixels_but_not_the_label(records):
    clean = ManifestDataset(records)[0]
    blurred = ManifestDataset(records, transform="blur_2.0")[0]
...
def test_paired_sample_shape_of_the_api(records):
    sample = PairedViewDataset(records, augment="jpeg_30")[0]
...
def test_paired_views_come_from_the_same_image_and_differ(records):
    sample = PairedViewDataset(records, augment="blur_2.0")[0]
...
def test_paired_views_keep_the_clean_branch_clean(records):
    dataset = PairedViewDataset(records, augment="jpeg_30")
...
def test_paired_augmentation_is_stochastic_across_accesses(records):
    ...
    dataset = PairedViewDataset(records, augment=RandomCompetitionTransform(seed=0))
    names = {dataset[0]["transform_name"] for _ in range(25)}
    assert len(names) > 1


def test_paired_augmentation_can_be_fixed_for_debugging(records):
    dataset = PairedViewDataset(records, augment="jpeg_50")
    assert {dataset[0]["transform_name"] for _ in range(5)} == {"jpeg_50"}
...
def test_paired_records_the_applied_transform_name(records):
    ...
    assert isinstance(dataset[0]["transform_name"], str) and dataset[0]["transform_name"]
...
def test_transformed_eval_dataset(records):
    dataset = TransformedEvalDataset(records, transform_name="jpeg_30")
    assert dataset.transform_name == "jpeg_30"
    assert dataset[0]["transform_name"] == "jpeg_30"
    assert "jpeg_30" in repr(dataset)
